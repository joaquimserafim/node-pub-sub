...
Publish.init = function (port, host, options) {
  ...
  this._server = new Netcat.server(this._port, this._host, {
    timeout: this._timeout, encoding: this._encoding
  });

  // events callbacks

  function subscribe (client, data) {
    var json = JSONValid(data.toString(self._encoding));

    // not valid, exit
    if (!json) throw "asasasas";
